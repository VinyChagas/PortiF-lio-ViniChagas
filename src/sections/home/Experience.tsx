      ...
      <Container className="relative">
        <Reveal>
          <SectionHeading
            index="05"
            label="Trajetória"
            description="Operação, software, automação e IA aplicada — nessa ordem, porque foi assim que o trabalho aconteceu."
          />
        </Reveal>

        <div ref={trackRef} className="relative z-10 mt-14 md:mt-16">
          <div
            aria-hidden="true"
                  ...
                  <Reveal delay={index * 0.06}>
                    <div className="grid grid-cols-[1.25rem_1fr] gap-x-5 md:grid-cols-[9rem_1.25rem_1fr] md:gap-x-8">
                      <div className="hidden pt-1 text-right md:block">
                        <p className="font-mono text-sm tracking-[0.18em] text-text-secondary">{year}</p>
                      ...
                      <div>
                        <p className="font-mono text-[0.62rem] tracking-[0.16em] text-brand-cyan uppercase">
                          {entry.stage}
                        </p>
                        <p className="mt-2 font-mono text-[0.66rem] tracking-[0.16em] text-text-muted uppercase md:hidden">
                        ...
                        <p className="mt-1 font-mono text-[0.62rem] tracking-[0.12em] text-text-muted uppercase">
                          {entry.location}
                        </p>
                          ...
                          <dl className="mt-5 flex flex-wrap gap-x-8 gap-y-3">
                            {entry.metrics.map((metric) => (
                              <div key={metric.label}>
                                <dt className="max-w-[16rem] font-mono text-[0.62rem] tracking-[0.12em] text-text-muted uppercase">
                                  {metric.label}
                                </dt>
        ...
        <div className="relative z-10 mt-16 border-t border-white/10 pt-10 md:mt-20">
          <h3 className="font-mono text-[0.68rem] tracking-[0.22em] text-text-muted uppercase">Formação</h3>
          <ul className="mt-4 divide-y divide-white/10">
            {education.map((item) => (
              <li key={item.id} className="grid gap-1 py-4 md:grid-cols-[9rem_1fr] md:gap-8">
                <p className="font-mono text-[0.66rem] tracking-[0.12em] text-text-muted">{item.period}</p>
                <div>
